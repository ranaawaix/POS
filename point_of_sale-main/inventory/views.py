...
class UpdatePurchaseOrderView(LoginRequiredMixin, AdminRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for instance in parent_instance:
            dict = {'product': instance.product, 'quantity': instance.quantity, 'price': instance.price,
                    'total': instance.total}
            initial_data.append(dict)
        context = super().get_context_data(object=self.object)
        purchase_order_item_formset = modelformset_factory(PurchaseOrderItem, form=PurchaseOrderItemForm,
                                                           can_delete=True,
                                                           extra=len(initial_data), error_messages=False, )
        if self.request.POST:
            context['purchase_order_item_formset'] = purchase_order_item_formset(self.request.POST,
